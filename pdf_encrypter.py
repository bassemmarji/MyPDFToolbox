...
def decipher_file(input_file:str, output_file:str, password: str):
    ...
    with open(input_file, mode='rb') as inp_buffer:
        try:
           #Decrypt Stream
           pyAesCrypt.decryptStream(inp_buffer, out_buffer, password, BUFFER_SIZE,inpFileSize)
...
def encrypt_decrypt_file(**kwargs):
    ...
    if action == "encrypt":
       result , pdf_reader , pdf_writer = encrypt_pdf(input_file = input_file, password= password)
       ...
       if result:
         ...
         if level == 2:
            decipher_file(input_file = input_file, output_file = output_file, password = password)

         result, pdf_reader, pdf_writer = decrypt_pdf(input_file=input_file, password=password)
...
def encrypt_decrypt_folder(**kwargs):
    """
    Encrypts all PDF Files within a specified path
    Decrypts all PDF Files within a specified path
    """
    ...
    #Protection Level
    #Level 1 --> Encryption / Decryption using PyPDF4
    #Level 2 --> Encryption / Decryption + Ciphering / Deciphering
    level = kwargs.get('level')
    ...
    for foldername, dirs, filenames in os.walk(input_folder):
        ...
        for filename in filenames:
            ...
            encrypt_decrypt_file(input_file=inp_pdf_file, output_file=None, password=password,action=action,level=level)
...
if __name__ == '__main__':
    ...
    # If Folder Path
         ...
         # Encrypting or Decrypting Folder
         encrypt_decrypt_folder(input_folder=args['input_path']
                              , password=args['password']
                              , action=args['action']
                              , level=args['level']
