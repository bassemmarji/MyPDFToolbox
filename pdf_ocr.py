...
def save_file_content(pdfContent,input_file):
    ...
    """
    content_file = os.path.join(os.path.dirname(input_file),os.path.splitext(os.path.basename(input_file))[0] + ".csv")
    ...
    for seq in range(len(details['text'])):
        ...
        if float(details['conf'][seq]) > 30.0:
            ...
            if highlight_readable_text:
                (x,y,w,h) = (details['left'][seq], details['top'][seq], details['width'][seq], details['height'][seq])
...
def ocr_file(**kwargs):
    ...
    dfResult   = pd.DataFrame(columns=['page','page_readable_items', 'page_matches', 'page_total_confidence'])
    ...
    for pg in range(pdfIn.pageCount):
        ...
        dfResult = dfResult.append({'page':(pg+1),'page_readable_items':pg_readable_items,'page_matches':pg_matches,'page_total_confidence':pg_total_confidence},ignore_index=True)

        if generate_output:
           pdfContent = save_page_content(pdfContent = pdfContent, page_id =(pg+1),page_data=pg_output_data)
        ...
        """
        pageo = pdfOut.newPage(pno=-1, width=page.rect.width, height=page.rect.height)
    ...
    content_file = None
    if generate_output:
        content_file = save_file_content(pdfContent=pdfContent, input_file=input_file)
    ...
    for foldername, dirs, filenames in os.walk(input_folder):
        for filename in filenames:
            ...
            if search_str:
               ...
               output_file = os.path.join(os.path.dirname(inp_pdf_file) , 'ocr_' + os.path.basename(inp_pdf_file))
...
if __name__ == '__main__':
    ...
    ocr_img(
              img=None
            , input_file="C:\\SCRIPTS\\Test\\Python Faker Library_0.PNG"
            , search_str="Python"
            , highlight_readable_text=False
            , action='Highlight'
    ...
    ocr_file(
             input_file="C:\\SCRIPTS\\Test\\MDSL_APPROVED.pdf"
           , output_file= None #"C:\\SCRIPTS\\Test\\MDSL_APPROVED_ocr.pdf"
           , search_str = None
           , pages=(0,1)
           , highlight_readable_text = False
           , action = 'Highlight'
    ...
    ocr_folder(
            input_folder = "C:\\SCRIPTS\\Test\\NodeJS"
          , recursive    = False
          , search_str   = "Python"
          , pages = (0)
          , action = "Highlight"
