...
def watermark_unwatermark_file(**kwargs):
    ...
    temporary, output_file = get_output_file(input_file, kwargs.get('output_file'))

    if action == "watermark":
       result , pdf_reader , pdf_writer = watermark_pdf(input_file = input_file, wm_text= wm_text, pages=pages)
    elif action == "unwatermark":
       result, pdf_reader, pdf_writer = unwatermark_pdf(input_file = input_file, wm_text= wm_text, pages=pages)
...
def watermark_unwatermark_folder(**kwargs):
    ...
    for foldername, dirs, filenames in os.walk(input_folder):
        for filename in filenames:
            ...
            watermark_unwatermark_file(input_file=inp_pdf_file, output_file=None, wm_text=wm_text,action=action,mode=mode,pages=pages)
...
def parse_args():
    ...
    parser.add_argument('-m'
                      ...
                      , type=str
                      , default='RAM'
                      , help="Choose whether to process on the hard disk drive or in memory")
...
if __name__ == '__main__':
    ...
    # If Folder Path
        ...
        watermark_unwatermark_folder(input_folder=args['input_path']
                                   , wm_text=args['watermark_text']
                                   , action=args['action']
                                   , mode=args['mode']
    ...
    #result , wm_buffer = create_watermark("CONFIDENTIAL")
   #output_file = os.path.dirname(os.path.abspath(__file__)) + "\static\watermark.pdf"
   #result = save_watermark(wm_buffer,output_file=output_file)

   #inp_pdf_file = ".\\static\\Python Faker Library.pdf"
   #watermark_unwatermark_file(input_file=inp_pdf_file,wm_text="CONFIDENTIAL",action="watermark",mode='RAM',output_file=None)
   #watermark_unwatermark_file(input_file=inp_pdf_file,wm_text="CONFIDENTIAL", action="unwatermark", mode='RAM',output_file=None)

   #wat_pdf_file = (os.path.splitext(inp_pdf_file)[0]) + '_wat.pdf'
   #watermark_unwatermark_file(input_file=inp_pdf_file,wm_text="CONFIDENTIAL",action="watermark",mode='HDD',output_file=wat_pdf_file)
   #unw_pdf_file = (os.path.splitext(inp_pdf_file)[0]) + '_unw.pdf'
   #watermark_unwatermark_file(input_file=wat_pdf_file,wm_text="CONFIDENTIAL", action="unwatermark", mode='HDD',output_file=unw_pdf_file)

   #watermark_unwatermark_folder(input_folder = "C:\\SCRIPTS\\Test", recursive = True,wm_text="CONFIDENTIAL", action="watermark", mode="HDD")
   #watermark_unwatermark_folder(input_folder = "C:\\SCRIPTS\\Test", recursive = True,wm_text="CONFIDENTIAL", action="unwatermark", mode="HDD")

   #watermark_unwatermark_folder(input_folder = "C:\\SCRIPTS\\Test", recursive = True,wm_text="CONFIDENTIAL", action="watermark", mode="RAM")
   #watermark_unwatermark_folder(input_folder = "C:\\SCRIPTS\\Test", recursive = True,wm_text="CONFIDENTIAL", action="unwatermark", mode="RAM")
